from monai.transforms import Compose, NormalizeIntensityd,Resized,AddChanneld,ToTensord, Orientationd, Spacingd, CastToTyped,ToTensor,AddChannel,CastToType
...
def transform_2d_image():
        transform = Compose([AddChanneld(keys=['raw', 'label']),
                             ...
                             # Resized(keys=['raw', 'label'], spatial_size=(256,256)),
                             # NormalizeIntensityd(keys=['raw']),
...
#         # transforms.Lambda(lambda x: x.type(torch.float32))  # Cast to float32.
#     ])
#     return transform
