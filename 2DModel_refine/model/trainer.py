...
from torch.nn import BCELoss, L1Loss, MSELoss, CrossEntropyLoss
...
def load_checkpoint(checkpoint_path, model, optimizer=None,model_key='model_state_dict', optimizer_key='optimizer_state_dict'):
...
def create_ARGAN_2d_trainer(config):
...
class ARGANTrainer:
    ...
    def train(self):
        ...
        for batch_idx, t in tqdm.tqdm(
            ...
            train_loss.update(arnet_loss,self._batch_size(lpet_images))
...
#             train_ssim.update(ssim,self._batch_size(lpet_images))
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, t in tqdm.tqdm(
                ...
                val_advnet_fake_output = self.discriminate(torch.cat((val_lpet_images, val_fake_images), dim=1))  #.detach()
                ...
                val_loss_pre.update(val_content_loss_prenet.item(), self._batch_size(val_lpet_images))
                  # (x = LPET; y = SPET; P = PreNet(),L1(y, P(x)))
                ...
                val_loss_ar.update(val_content_loss_arnet.item(), self._batch_size(val_lpet_images))
                  # (r = SPET - P(x); R = ARNet(P(x)); R(x) = R*P(x), L1(r, R(x)) )

                val_arnet_loss = val_adv_loss.item() + val_content_loss_prenet.item() + val_content_loss_arnet.item()
                val_loss.update(val_arnet_loss,self._batch_size(val_lpet_images))
            ...
            logger.info(
                f"Validation Loss: {val_loss.avg:.4f}, val_psnr: {val_psnr.avg}, val_mse: {val_mse.avg}, val_nrmse: {val_nrmse.avg}, val_ssim: {val_ssim.avg}")  # Print the average validation loss
            self._log_stats('validation', val_loss.avg, val_arnet_loss, val_loss_pre.avg,
                            val_loss_ar.avg, val_loss_adv.avg, val_psnr.avg, val_mse.avg,
                            val_nrmse.avg, val_ssim.avg)
            # self._log_stats('validation', val_loss.avg, 0, val_loss_pre.avg,
            #                 val_loss_ar.avg, val_loss_adv.avg, 0, val_psnr.avg, val_mse.avg,
            #                 val_nrmse.avg, val_ssim.avg)
    ...
    def _log_params(self):
        ...
        for name, value in self.refine.named_parameters():
            # print(name,value)
            self.writer.add_histogram(name, value.data.cpu().numpy(), self.num_iterations)
        for name, value in self.discriminate.named_parameters():
            # print(name,value)
            self.writer.add_histogram(name, value.data.cpu().numpy(), self.num_iterations)
...
def create_DRFARGAN_2d_trainer(config):
    ...
    # model = torch.nn.DataParallel(model, device_ids=device_ids
    if torch.cuda.device_count() > 1 and not device.type == 'cpu':
        generate_models = [nn.DataParallel(model).to(device) for model in generate_models] #, device_ids=device_ids)
        refine_models = [nn.DataParallel(model).to(device) for model in refine_models] #, device_ids=device_ids)
        discriminate_models = [nn.DataParallel(model).to(device) for model in discriminate_models] #, device_ids=device_ids)
        logger.info(f'Using {torch.cuda.device_count()} GPUs for training')
    else:
        generate_models = [model.to(device) for model in generate_models]
        refine_models = [model.to(device) for model in refine_models]
        discriminate_models = [model.to(device) for model in discriminate_models]
    ...
    # print('config',config)
    loaders = get_train_loaders(config)
    optimizers_pre = {drf: create_optimizer(config['optimizer'], generate_models[drf].parameters()) for drf in drf_list}
    optimizers_refine = {drf: create_optimizer(config['optimizer'], refine_models[drf].parameters()) for drf in drf_list}
    optimizers_disc = {drf: create_optimizer(config['optimizer'], discriminate_models[drf].parameters()) for drf in drf_list}
