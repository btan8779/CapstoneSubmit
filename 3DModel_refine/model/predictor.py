...
from data.datasets import H5Dataset
from data.dataloader import SliceBuilder
...
def _get_output_file(suffix='_predictions', output_dir=None,file_path = None):
...
class StandardPredictor(_AbstractPredictor):
    ...
    def __call__(self, test_loader):
        # print(test_loader.dataset.file_path)
        # assert isinstance(test_loader.dataset, H5Dataset)

        file_path = self.config['loaders']['test']['file_paths'][0]

        logger.info(f"Processing '{file_path}'...")
        
        output_file = _get_output_file(output_dir=self.output_dir,file_path = file_path)
        # print(output_file)
        ...
        volume_shape = self.volume_shape(test_loader[0])
        ...
        with torch.no_grad():
                raw_batch,label_batch, indices = test_loader
                # print(data)
                # raw_batch,label_batch, indices = data[0],data[1],data[2]
                # batch = data[0]
                # indices = data[1]
            # for batch, indices in test_loader: #, indices
                ...
                # forward pass
                # only for ARNetV2
                pre_output = self.model(raw_batch)
                preliminary_output = pre_output.detach()
                # residual = preliminary_output - batch
                # refine_input = preliminary_output-raw_batch
                estimated_res = preliminary_output*self.refine_model(preliminary_output)
                ...
                for prediction, prediction_map, normalization_mask in zip(predictions, prediction_maps,
                    ...
                    for pred, index in zip(prediction, indices.detach().numpy()):
