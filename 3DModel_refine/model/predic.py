...
from data.datasets import H5Dataset,StandardHDF5Dataset,AbstractHDF5Dataset,SliceBuilder,SequentialHDF5Dataset
    ...
    def __call__(self, test_loader):
        ...
        with torch.no_grad():
            ...
            # for input, pre_output, indices in test_loader:
                ...
                rectified_input = preliminary_output - batch
                # estimated_res = preliminary_output*self.refine_model(preliminary_output)
                # rectified_input = (preliminary_output - batch)/(batch+ 1e-10)
                estimated_res = self.refine_model(rectified_input)
                predictions = preliminary_output + estimated_res
...
class SequentialPredictor(_AbstractPredictor):
    def __init__(self, refine_model, model, output_dir, config, **kwargs):
        super().__init__(refine_model, model, output_dir, config, **kwargs)

    def __call__(self, test_loader):
        assert isinstance(test_loader.dataset, SequentialHDF5Dataset)
        ...
        # Sets the module in evaluation mode explicitly
        # It is necessary for batchnorm/dropout layers if present as well as final Sigmoid/Softmax to be applied
        for model_name, gen in self.model.items():
            gen.eval()
            self.refine_model[model_name].eval()
        with torch.no_grad():
            for batch, indices in test_loader:
                # for input, pre_output, indices in test_loader:
                # send batch to device
                ...
                predictions = self.forward_pass(batch)
                ...
                for prediction, prediction_map, normalization_mask in zip(predictions, prediction_maps,
                    ...
                    for pred, index in zip(prediction, indices):
                        ...
                        if prediction_channel is not None:
                            # use only the 'prediction_channel'
                            logger.info(f"Using channel '{prediction_channel}'...")
                            # for drf, predict in prediction.items():
                            pred = np.expand_dims(pred[prediction_channel], axis=0)
    ...
    def forward_pass(self, input_data):
        ...
        for model_name, gen in self.model.items():
            ...
            #             print('gen output')
            rectified_parameters = self.refine_model[model_name](preliminary_predictions)
            #             print('refine')
        ...
        return generate_images['Full_dose']
