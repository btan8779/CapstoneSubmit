...
from data.trans import transform_3d_image,transform_image
import numpy as np
import torch
...
class H5Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.image_list[index], 'r') as input_file:
            ...
            trans_raw = self.transform(self.raw)
            trans_label = self.transform(self.label)
...
class H5DatasetSequential(Dataset):
    def __init__(self, file_path,
                 phase,
                 raw_internal_path='raw',
                 label_internal_path='label',
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.image_list[index],'r') as input_file:
            ...
            if self.phase == 'test':
                ...
                trans_label = trans_data['label']['Full_dose']
                ...
                for drf in drf_list:
                    trans_label[drf] = self.transform(self.label[drf])
...
class AbstractHDF5Dataset(ConfigDataset):
    ...
    def __init__(self, file_path,
        ...
        self.transformer = transform_image()
        ...
        # print(self.raw_slices[0])
        # print(self.label_slices)
        # print('raw',len(self.raw_slices))#,self.raw_slices[0].shape)
        # print('label',len(self.label_slices))#,self.label_slices[0].shape)
    ...
    def __getitem__(self, idx):
        ...
        # print(self.raw_transform)
        # print(self.raw[raw_idx])
...
class SequentialHDF5Dataset(ConfigDataset):
    """
    Implementation of torch.utils.data.Dataset backed by the HDF5 files, which iterates over the raw and label datasets
    patch by patch with a given stride.
    """
...
class LazyHDF5Dataset(AbstractHDF5Dataset):
    ...
    def __init__(self, file_path, phase, slice_builder_config, transformer_config, mirror_padding=(16, 32, 32),
                 raw_internal_path='raw', label_internal_path='label', pre_prediction_path='predictions', weight_internal_path=None,
