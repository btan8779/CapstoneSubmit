...
class PreNet(nn.Module):
    def __init__(self, in_channels, out_channels,**kwargs):
        ...
        # print('first_in', in_channels)
        self.first_conv = nn.Sequential(
            nn.Conv3d(in_channels, in_channels * 2, kernel_size=3,stride = (1,2,2),  padding=1))
        in_channels *= 2
        ...
        for _ in range(6):
            ...
            self.decoder.append(nn.Sequential(
                nn.ConvTranspose3d(in_channels*2, in_channels // 2, kernel_size=3, stride=(1,2,2), padding=1, output_padding=(0,1,1)),
                nn.BatchNorm3d(in_channels // 2),
        ...
        self.final_conv = nn.Sequential(
            nn.ConvTranspose3d(in_channels, out_channels, kernel_size=3, stride=(1,2,2), padding=1, output_padding=(0,1,1)))
        # print('fil', out_channels)

    def forward(self, x):
        encoder_outs = []
        # print('first_in', x.shape)
        x = self.first_conv(x)
...
class ARNet(nn.Module):
    def __init__(self, in_channels, out_channels,**kwargs):
        ...
        self.contr_1 = nn.ConvTranspose3d(64,64, kernel_size=2, stride=2)
        self.contr_2 = nn.ConvTranspose3d(32,32, kernel_size = 2, stride = 2)
