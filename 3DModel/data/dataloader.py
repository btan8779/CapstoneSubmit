...
#     test_file_path = loaders_config['test']['file_paths'][0]

#     test_datasets = dataset_class(file_path=test_file_path, phase='test')
#     # print(test_datasets[0])
#     # print(test_datasets[1])
#     # print(test_datasets[2])
#     print(len(test_datasets))
#     # print('dataset:',test_datasets.shape)
#     # op
...
#         # print(len(test_datasets))
#         # print(test_dataset)
#         # print(test_dataset.file_path)
#         # print(test_dataset.shape)
#         logger.info(f'Loading test set from: {test_file_path}...')
#         if hasattr(test_dataset, 'prediction_collate'):
#             collate_fn = test_dataset.prediction_collate
#         else:
#             collate_fn = default_prediction_collate
#     # return DataLoader(test_datasets, batch_size=batch_size, num_workers=num_workers)
#         yield DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers,
#                         collate_fn=collate_fn)
...
#     #     # Separate the 'raw' and 'label' from each dictionary in the batch
#     #     raw = [item['raw'] for item in batch]
#     #     label = [item['label'] for item in batch]
#     #     # Stack them separately
#     #     return {'raw': torch.stack(raw, 0), 'label': torch.stack(label, 0)}
#     if isinstance(batch, torch.Tensor):
#         return torch.stack(batch, 0)
#     elif isinstance(batch, tuple) and isinstance(batch[0], slice):
#         return batch
#     elif isinstance(batch, _collections_abc.Sequence):
#         transposed = zip(*batch)
#         print(transposed)
#         for samples in transposed:
#             print(samples)
#         return [default_prediction_collate(samples.item()) for samples in transposed]
