...
from model.losses_2d import SpectralLoss
# from model.losses_v4 import SpectralLoss
# from model.loss_radon import SpectralLoss
...
def create_ARGAN_2d_trainer_with_spectrum(config):
...
class ARGANTrainer_with_spectrum:
    ...
    def train(self):
        ...
        for batch_idx, t in tqdm.tqdm(
            ...
            # print(f'spectral_loss_rectified: {spectral_ai_rectified.shape}; spectral_loss_real: {spectral_ai_real.shape}')
            #
            spectral_loss_total = self.lambda_specturm * self.adv_loss(spectral_ai_rectified,spectral_ai_real)
            # del spectral_ai_real,spectral_ai_rectified
            # print(spectral_loss_total)
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch_idx, t in tqdm.tqdm(
                ...
                val_spectral_ai_rectified = SpectralLoss(val_rectified_spet_like_images)
                ...
                # val_loss_spectrum+= val_spectral_loss_total.item()
                val_loss_spectrum.update(val_spectral_loss_total.item(), self._batch_size(val_lpet_images))
                # del val_spectral_ai_real, val_spectral_ai_rectified
                ...
                val_loss_ar.update(val_content_loss_arnet.item(), self._batch_size(val_lpet_images))
                  ...
                  # spectral_loss_rectified = spectral_reg_loss(rectified_spet_like_images)
                  # spectral_loss_real = spectral_reg_loss(real_images)
                val_arnet_loss = val_adv_loss.item() + val_content_loss_prenet.item() + val_content_loss_arnet.item() + val_spectral_loss_total.item()
                # del val_spectral_loss_total
            ...
            self._log_stats('validation', val_loss.avg, val_arnet_loss, val_loss_pre.avg,
                            val_loss_ar.avg, val_loss_adv.avg, val_loss_spectrum.avg, val_psnr.avg, val_mse.avg,
...
def create_ARGAN_2d_trainer_without_spectrum(config):
    # Get the model we need
    generate_model = get_model(config['generator'])
    refine_model = get_model(config['refine_model'])
    discriminate_model = get_model(config['discriminator'])
    ...
    return ARGANTrainer_without_spectrum(
                         generate_model=generate_model,
                         refine_model=refine_model,
                         dis_model=discriminate_model,
                         optimizer_pre=optimizer_pre,
